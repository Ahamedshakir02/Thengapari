...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  React.useLayoutEffect(() => {
    ...
    // First load (no saved viewport): fit all artboards into view so every
    // screen is visible at once instead of landing at 100% on the top-left.
    if (!restored) {
      requestAnimationFrame(() => {
        const vp = vpRef.current;
        if (!vp) return;
        const slots = vp.querySelectorAll('[data-dc-slot]');
        if (!slots.length) return;
        const vr = vp.getBoundingClientRect();
        const { x: tx, y: ty, scale: ts } = tf.current;
        // Union of artboard rects, converted from screen space back to world
        // (unscaled) coordinates — robust against the infinite-canvas size.
        let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
        slots.forEach((s) => {
          const r = s.getBoundingClientRect();
          const wx = (r.left - vr.left - tx) / ts, wy = (r.top - vr.top - ty) / ts;
          const ww = r.width / ts, wh = r.height / ts;
          minX = Math.min(minX, wx); minY = Math.min(minY, wy);
          maxX = Math.max(maxX, wx + ww); maxY = Math.max(maxY, wy + wh);
        });
        const cw = maxX - minX, ch = maxY - minY;
        if (!cw || !ch) return;
        const fit = Math.min(vr.width / cw, vr.height / ch) * 0.9;
        const scale = Math.min(maxScale, Math.max(minScale, fit));
        tf.current = {
          x: (vr.width - cw * scale) / 2 - minX * scale,
          y: Math.max(20, (vr.height - ch * scale) / 2 - minY * scale),
          scale,
        };
        apply();
      });
