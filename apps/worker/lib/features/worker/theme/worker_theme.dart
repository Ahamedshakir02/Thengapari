...
/// Worker palette (dark teal) — mirrors the `--w-*` tokens in the worker design
/// system (`Designs/ThengaPari Worker App/index.html`). Shared across every
/// worker screen so they stay visually consistent.
class WColors {
  WColors._();
...
class WSectionHead extends StatelessWidget {
  ...
  final bool dense;
  const WSectionHead(
      {super.key, required this.title, this.trailing, this.dense = false});
...
class WRupee extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Text('₹$value',
        style: AppText.displayNum(size, color: color, weight: weight));
  }
}
